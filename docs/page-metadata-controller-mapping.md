...
|              | all          | 复制+导出（两个按钮） | `ClusterCycleController.createClusterByCopy`；`POST /organization/clusterCycleDeploy/createClusterByCopy`                                                      | 后端已有；当前页面使用本地复制任务，没有接入后端。                     |
...
|              | relationship | 建立关联              | E:\EventMesh\eventmesh-dashboard\eventmesh-dashboard-console\src\main\java\org\apache\eventmesh\dashboard\console\controller\deploy\ClusterCycleController.java    | 后端接口存在但已标记`@Deprecated`；当前页面调用 `addRelations`。   |
|              | relationship | 解除关联              | E:\EventMesh\eventmesh-dashboard\eventmesh-dashboard-console\src\main\java\org\apache\eventmesh\dashboard\console\controller\deploy\ClusterCycleController.java    | 后端接口存在但已标记`@Deprecated`；当前页面调用 `removeRelation`。 |
