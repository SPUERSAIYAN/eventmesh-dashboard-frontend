...
- Inside a selected cluster, keep a standalone `概览` destination above the `EventMesh 集群` group. It summarizes the current EventMesh together with every associated Runtime, Meta, Kafka, and RocketMQ cluster; the nested `EventMesh 集群 / 概要` remains the EventMesh-specific operational page. This is cluster-scoped and does not add a global overview to the all-clusters inventory shell.
...
- Model EventMesh as a dependency root that can associate with multiple independently managed Runtime, Meta, Kafka, and RocketMQ component clusters; each component cluster may be shared by multiple EventMesh clusters.
- Keep Runtime, Meta, and storage as separate first-level cluster-scoped navigation groups. Broker is not a first-level module: Kafka Broker belongs to Kafka storage clusters and RocketMQ Broker belongs to RocketMQ storage clusters.
- EventMesh provides overview, topology, and relation-list destinations. Runtime provides overview, cluster list, Runtime list, and reverse-relation list; Meta provides overview, cluster list, and reverse-relation list; storage provides overview, Kafka list, RocketMQ list, and reverse-relation list.
- Relationship creation and removal are frontend-only mock interactions with versioned local persistence. Relationship pages must make direction explicit: EventMesh depends on components, while component pages show the EventMesh clusters that reference them.
- Kafka and RocketMQ inventory rows open distinct nested storage-cluster control panels inside the existing EventMesh shell. Each storage console provides overview, Broker, physical Topic, consumer-group, and related-EventMesh destinations.
- Runtime and Meta inventory rows also open distinct nested component-cluster control panels inside the existing EventMesh shell. Runtime consoles provide overview, Runtime instances, client connections, EventMesh Topics/subscriptions, and related EventMesh destinations; Meta consoles provide overview, Meta nodes, Runtime registry information, and related EventMesh destinations.
- Frontend mock write actions are available at their owning resource pages: component relations on EventMesh relation pages; Runtime instances, Meta nodes, and storage Brokers inside their selected cluster consoles; business Topics from Topic overview/list with an associated storage target; physical Topics inside a selected Kafka/RocketMQ console; and Consumer Groups on the Consumer page. Business Topic creation must also create the corresponding physical Topic entry in the selected storage cluster.
- Keep the semantic boundary explicit: the cluster-scoped top-level Topic module represents EventMesh-facing message topics, while Topic pages inside a Kafka or RocketMQ console represent the physical topics stored in that selected middleware cluster.
